...
<body>
		...
		<div class="container">
			<div class="row align-items-center">
				...
				<div class="a-page">
					<div class="form">
						<div style="width: 100%">
							<form action="searchPw.do" class="pw-form" id="find-form"
								...
								<fieldset class="pw">
									...
									<div class="phone">
										<div style="display: flex; justify-content: space-between;">
											<input style="width: calc(70% - 10px);" id="pp"
												class="userPhoneNum" type="tel" name="phone"
												placeholder="phone number"
												pattern="[0-9]{3,3}-[0-9]{4,4}-[0-9]{4,4}" maxlength="13"
												required />
											<button onclick="sms()" type="button" id="ppBtn"
												style="height: 55px; width: 30%;">인증번호 받기</button>
										</div>
										<div style="display: flex; justify-content: space-between;">
											<input style="width: calc(70% - 10px);" type="text"
												placeholder="인증번호를 입력하세요" id="userCheck" maxlength="5"
												required>
											<button onclick="smsCheck()" id="chkBtn" type="button"
												style="height: 55px; width: 30%;">인증번호 확인</button>
										</div>
									</div>
	...
	<script type="text/javascript">
		...
		$inputPhone.addEventListener("input", function(event) {
			...
			this.value = this.value.replace(/^(\d{2,3})(\d{3,4})(\d{4})$/,
					`$1-$2-$3`);
						...
						$.ajax({
							...
							success : function(randNum) {
								if (randNum != null) {
									...
									alert("인증번호 전송이 불가능합니다..");
								}

							}
						})
					} else {
						alert("아이디랑 전화번호가 다릅니다.");
					}
